...
include 'exportX.php';
	...
	error_log("macchinexOP.php"." ".$oper);
	...
	switch ($oper){
		case "insertOP":
				...
				if ($res) {
					...
					if ($res) {
						$commessa=trovaValoreCampo($valoreCampoCommessa,$campoCommessa,"id_order");
						$pn=trovaValoreCampo($valoreCampoCommessa,$campoCommessa,"prodotto");
						$res=valorizzaMacchinexcommessa($commessa,$pn,$valoreCampoOP,$campo,$valoreCampo);
					}
				}
		...
		case "getListxM":
			...
			if(isset($_POST["op"]))
			{
				...
				if (isset($_POST["ordinamento"])) $sort=$_POST["ordinamento"];//first, last,nextfw,nextbw
			}
			else
			{
				...
				if (isset($_GET["ordinamento"])) $sort=$_GET["ordinamento"];//first, last,nextfw,nextbw
			}
		...
		error_log("pag:".$pag." npag:".$npag." filtro:".$filtro);

			//-1-$campiElenco="mc.id_order,mc.unique_id, m.posizione, mc.product,    mc.requested_pieces, mc.CNCprogram, mc.planned_start_date, p.order_start_date,  p.order_start_pieces, mc.planned_end_date, p.order_end_date,  p.total_pieces"; 
			//-1-$tabelleJoin="macchinexcommessa as mc left outer join pcs as p  on  mc.unique_id=p.unique_id and mc.CNCprogram=p.order_start_program and mc.planned_start_date <= p.order_start_date and mc.planned_end_date >= p.order_end_date left outer join macchina as m on mc.unique_id=m.unique_id";
			//-2-$campiElenco="mc.id_order,mc.unique_id, m.posizione, mc.product,    mc.requested_pieces, mc.CNCprogram, mc.planned_start_date, mc.planned_end_date"; 
			//-2-$tabelleJoin="macchinexcommessa as mc  left outer join macchina as m on mc.unique_id=m.unique_id";
			/*
			select m.posizione,mc.* 
			from 
			(select unique_id, max(planned_start_date) as maxdata from macchinexcommessa group by unique_id) as rs
			inner join macchinexcommessa as mc on rs.unique_id=mc.unique_id
			join macchina as m on mc.unique_id=m.unique_id
			where rs.maxdata=mc.planned_start_date
			order by posizione
			...
			*/
			...
			$tabelleJoin="(select unique_id, max(planned_start_date) as maxdata from macchinexcommessa group by unique_id) as rs
			inner join macchinexcommessa as mc on rs.unique_id=mc.unique_id
			join macchina as m on mc.unique_id=m.unique_id";
			...
			if ($oper !="getListxOP"){
				...
				$filtro .= " rs.maxdata=mc.planned_start_date ";
			}
	...
	if ( $oper == "list" || $oper == "listbox" || $oper =="getListxOP" || $oper =="getListxM" || $oper == "get" || $oper =="getCNCcounter")
...
function valorizzaCommessa($dbTH,$dbhandle,$valoreCampoOP,$nomeCampoCommessa,&$valoreCampoCommessa)
{
	...
	*/
	...
	// da articolo ricava part-number:
	// cerca nella tabella partnumbers, colonna descrizione
	// se non trovato procede alla rimozione di eventuali prefissi separati da spazio o -
	// se il prefisso è EX la stringa va invertita
	$npag=1;
	$pag=1;
	$nrighexpag=1;
	$ordinamento="";$raggruppamento="";
	$filtro="descrizione='".$prodotto_OP."'";
	$row=$dbTH->db_get($dbhandle,"partnumbers","part_number,descrizione",$filtro,$pag,$nrighexpag,$ordinamento,$raggruppamento,$npag);

	$trovato=(count($row)>0);
	if ($trovato) {
		$partnumber=$row[0]["part_number"];
	}
	else {
	...
	if ($i>1 &&  $prodotto[0]=="EX")
		$partnumber=strrev($partnumber);
	}
...
function trovaValoreCampo($valoreCampo,$nomeCampo,$campo){
	$iTrovato=-1;
	$e=0;
	for ($i=0;$i<count($nomeCampo);$i++){
		if (($e=strcmp($nomeCampo[$i],$campo))==0) {
			$iTrovato=$i;
		}
		
	}
	if ($iTrovato>=0)
		return $valoreCampo[$iTrovato];
	else
		return "";
}
function valorizzaMacchinexCommessa($id_order,$partnumber,$valoreCampoOP,$nomeCampoMxC,&$valoreCampoMxC)
{
	$res=true;
	// valorizza commessa
	/*
	...
	*/
	...
	$partnumber=$prodotto[$i-1];
	*/
	error_log("valorizzaMacchinexCommessa - id_order:$id_order product:$partnumber");
